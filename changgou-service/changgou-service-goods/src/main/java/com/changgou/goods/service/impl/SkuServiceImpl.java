...
public class SkuServiceImpl implements SkuService {
     ...
     */
    @Override
    public void decrCount(Long skuId, Integer num) {

        int decrCount = skuMapper.decrCount(skuId, num);
        if (decrCount <= 0) {
            new RuntimeException("超过库存了亲");
        }
     ...
     */
    @Override
    public List<Sku> findSkuList(String status) {
        Sku sku = new Sku();
