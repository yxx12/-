...
public class SpuController {
     ...
     */
    @PutMapping("/audit")
    public Result audit(@RequestParam(value = "ids") Long[] ids,
                        @RequestParam(value = "status") String status){
        Integer count = spuService.audits(ids, status);
        return new Result(true, StatusCode.OK, "批量审核成功个数："+count);
    }
    /**
     * 单个修改审核状态
     * @param id
    ...
    public Result findGoodBySpuId(@PathVariable(value = "spuId") Long spuId) {
        Goods goods = spuService.findGoodBySpuId(spuId);
        return new Result(true, StatusCode.OK, "查找成功", goods);
    }

    @PostMapping("/saveOrUpdate")
    public Result saveOrUpdate(@RequestBody Goods goods) {
        spuService.saveOrUpdateGoods(goods);
        return new Result(true, StatusCode.OK, "保存/修改 成功");
    }
