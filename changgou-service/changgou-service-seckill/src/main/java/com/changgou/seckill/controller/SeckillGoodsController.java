...
public class SeckillGoodsController {
    ...
    @GetMapping("/detail/{time}/{id}")
    public Result detail(@PathVariable(value = "time") String time,
                         @PathVariable(value = "id") Long id) {
        SeckillGoods seckillGoods = seckillGoodsService.detail(time, id);
        return new Result(true, StatusCode.OK, "获取秒杀商品详情", seckillGoods);
    }

    @GetMapping("/list/{time}")
    public Result findSeckillGoodsListByTime(@PathVariable(value = "time") String time) {
        List<SeckillGoods> list = seckillGoodsService.findSeckillGoodsListByTime(time);
